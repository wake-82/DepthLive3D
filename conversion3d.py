...
def detect_scene_cuts_transnetv2(video_path: Path, device="cuda", threshold=0.5, log_fn=None,
    ...
    try:
        ...
        with torch.inference_mode():
            for start in range(0, T, window // 2):
                if stop_event is not None and stop_event.is_set():
                    if log_fn:
    ...
    def _run_convert(self, p: dict):
        ...
        try:
            ...
            if self._stop.is_set():
                ...
                self.finished_error.emit("Conversion was aborted.")
                return
...
class StereoVideoGUI(QMainWindow):
    def __init__(self):
        ...
        self.lbl_ema = QLabel()
        self._EMA_LEVEL_VALUES = [0.0, 0.20, 0.40, 0.60]
    ...
    def _preset_load(self):
        if self.chk_auto_mode.isChecked():
            QMessageBox.warning(
                self,
                "Notice",
                "Please turn off Auto Mode before loading a preset."
            )
            return
        name = self.cmb_preset_name.currentText()
