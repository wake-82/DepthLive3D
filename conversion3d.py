...
# Custom edge-fix level -> (x, y) mapping (matches live3d.py).
...
class ConvertWorker(QThread):
    ...
    def _run_convert(self, p: dict):
        ...
        edge_dilation = edge_fix_level_to_xy(edge_dilation) if not isinstance(edge_dilation, (list, tuple)) else edge_dilation
...
class StereoVideoGUI(QMainWindow):
    def __init__(self):
        ...
        self.lbl_ema = QLabel()
        self._EMA_LEVEL_VALUES = [0.0, 0.30, 0.40, 0.50]
    ...
    def _start(self):
        ...
        try:
            isz = int(self.cmb_input_size.currentText().strip())
            isz = max(64, min(630, int(round(isz / 32.0) * 32)))
